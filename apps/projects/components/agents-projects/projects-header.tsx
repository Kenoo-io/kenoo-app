...
/** My Tasks / All Projects controls — used in the tasks toolbar. */
export function ProjectsBoardFilters({
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (
        taskScopeDropdownRef.current &&
        !taskScopeDropdownRef.current.contains(e.target as Node)
      ) {
        setTaskScopeDropdownOpen(false);
      }
    }
    if (dropdownOpen || taskScopeDropdownOpen) {
      document.addEventListener("mousedown", handleClickOutside);
    }
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [dropdownOpen, taskScopeDropdownOpen]);

  return (
    <div className={cn("flex items-center gap-x-1.5 min-w-0", className)}>
      {showTaskScopeDropdown ? (
        <div className="relative min-w-0" ref={taskScopeDropdownRef}>
          <button
            ...
            className={cn(
              FILTER_TRIGGER,
              "text-sm font-light uppercase tracking-wider text-neutral-800",
            )}
          >
