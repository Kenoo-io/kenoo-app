...
interface ViewOutOfOfficeProps {
  eventData: {
    startTime: Date;
    endTime: Date;
...
export function ViewOutOfOffice({ eventData, isEditing = false, onDataChange }: ViewOutOfOfficeProps) {
  // Local state for editable fields
  const [localEventData, setLocalEventData] = useState({
    startTime: eventData.startTime,
    endTime: eventData.endTime,
    declineMeetings: eventData.declineMeetings
  });
  ...
  useEffect(() => {
    console.log('ViewOutOfOffice received data:', {
      startTime: eventData.startTime,
      endTime: eventData.endTime,
      declineMeetings: eventData.declineMeetings
    });
    ...
    <div data-event-type="outOfOffice" className="out-of-office-content">
      <div className="flex gap-4">
        ...
        <div className="flex-1 grid gap-6">
          ...
          {/* Decline meetings setting */}
          <div className="flex items-center justify-between border-0 border-b border-gray-200 py-[2px] mt-0">
            <span className="text-sm font-normal text-gray-500">Automatically decline meetings</span>
            {isEditing ? (
              <Switch
                checked={localEventData.declineMeetings}
                onCheckedChange={(checked) => {
                  setLocalEventData({
                    ...localEventData,
                    declineMeetings: checked
                  });
                }}
              />
            ) : (
              <span className="text-sm font-medium text-gray-700">
                {localEventData.declineMeetings ? "Yes" : "No"}
              </span>
            )}
          </div>
        </div>
