...
export async function listAllOrganizations(): Promise<OrganizationRecord[]> {
  ...
  if (error) {
    console.error("[admin] list organizations:", error);
    return [];
  }

  return (data ?? []).map((row) =>
    mapOrganization(row as AccountRow, {
      role: "owner",
      is_default: false,
    }),
  );
...
export async function getOrganizationById(
  ...
  if (error || !data) {
    if (error) console.error("[admin] get organization:", error);
    return null;
  }

  return mapOrganization(data as AccountRow, {
    role: "owner",
    is_default: false,
  });
}
...
export async function listOrganizationsForUser(
  ...
  if (error) {
    console.error("[admin] list organizations for user:", error);
    return [];
  }
...
export async function getOrganizationForUser(
  ...
  if (error || !data) {
    if (error) console.error("[admin] get organization for user:", error);
    return null;
  }
...
export async function isOrganizationSlugAvailable(
  ...
  if (error) {
    console.error("[admin] check organization slug:", error);
    return false;
  }
...
export async function updateOrganization(
  ...
  if (error) {
    console.error("[admin] update organization:", error);
    if (error.code === "23505") {
      return { ok: false, error: "This slug is already taken" };
    }
...
export async function setDefaultOrganizationForUser(
  ...
  if (clearError) {
    console.error("[admin] clear default organization:", clearError);
  ...
  if (setError) {
    console.error("[admin] set default organization:", setError);
    return { ok: false, error: "Failed to update default organization" };
  }
...
export async function deleteOrganization(
  ...
  try {
    ...
    console.warn("[admin] delete organization icons:", error);
  }
  ...
  if (error) {
    console.error("[admin] delete organization:", error);
