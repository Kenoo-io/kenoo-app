...
import {
  getApolloPersonPreview,
  runApolloPersonSync,
  type PersonSyncPreview,
} from "@/components/agentCRM/agentPeople/lib/apollo-person-sync";
...
export function ContactSearchApolloEnrich({ onSuccess, onClose }: ContactSearchApolloEnrichProps) {
  const [enrichInput, setEnrichInput] = React.useState("");
  const [isSyncing, setIsSyncing] = React.useState(false);
  const [showParsedResult, setShowParsedResult] = React.useState(false);

  const preview: PersonSyncPreview | null = React.useMemo(
    () => getApolloPersonPreview(enrichInput),
    [enrichInput]
  );
  ...
  const sync = React.useCallback(async () => {
    if (!preview) {
      wallsToast.error("Please enter a valid Apollo URL, person ID, contact ID, or email");
      return;
    }
    setIsSyncing(true);
    try {
      const result = await runApolloPersonSync(enrichInput);
      if (result.ok === false) {
        wallsToast.error(result.error);
        return;
      }
      wallsToast.success(result.message, {
        description: result.personName ?? undefined,
        duration: 3000,
      });
      setEnrichInput("");
      onSuccess?.({ personName: result.personName, personId: result.personId });
    } finally {
      setIsSyncing(false);
    }
      ...
      <div className="relative w-full">
        {preview ? (
          ...
          <div className={cn("relative min-h-[2.125rem] w-full border-b border-neutral-200 py-2", padRight)}>
            <p className="truncate text-sm font-light text-red-600">
              invalid — paste apollo url, id, or email
            </p>
            {trailingActions}
          </div>
        ) : (
          <div className="relative">
            <input
              ...
              placeholder="paste apollo url, id, or email"
              className={cn(
