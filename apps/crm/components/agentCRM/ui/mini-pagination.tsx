...
interface CRMPaginationProps {
  ...
  /** When true, renders without outer border/rounded/shadow so it can sit inside a table container */
  embedded?: boolean;
}

export function CRMPagination({ currentPage, totalPages, onPageChange, embedded }: CRMPaginationProps) {
  ...
  const buttonBaseClass = "min-w-[2.5rem] h-8 px-3 rounded-full bg-white/50 backdrop-blur-sm hover:bg-white/70 shadow-sm border border-neutral-200/50 transition-all duration-300 text-neutral-700";
  const arrowButtonClass = `${buttonBaseClass} w-8 p-0 disabled:opacity-50`;

  return (
    <div
      className={
        embedded
          ? "border-t border-neutral-200 bg-white/80 py-2 px-6"
          : "sticky bottom-0 left-0 right-0 z-50 bg-white/50 backdrop-blur-sm border border-neutral-200/50 shadow-sm rounded-b-lg py-1 px-8"
      }
    >
            ...
            <div key={`ellipsis-start`} className="relative flex items-center">
              {showStartInput ? (
                <Input
                  ...
                  className="w-20 h-8 text-center rounded-full bg-white/50 backdrop-blur-sm shadow-sm border border-neutral-200/50"
                  placeholder="Go to"
                  autoFocus
                />
              ) : (
