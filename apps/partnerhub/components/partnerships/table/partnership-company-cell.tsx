...
interface PartnershipCompanyCellProps {
...
export function PartnershipCompanyCell({
  partnership,
  companyFailed,
  companySelectOpen,
  isUpdating,
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (partnership.companyId) {
      onCompanyClick(partnership.companyId, e);
      return;
    }

    if (partnership.companyWebsite) {
      window.open(ensureHttps(partnership.companyWebsite), "_blank");
    }
  };

  const companyLogo =
    partnership.companyLogo && !companyFailed ? (
      <Image
        ...
        <button
          ...
          title={
            partnership.companyId
              ...
              : "Click to open website · hold to link company"
          }
          className={cn(
